...
from src.params import DeQADataArguments
...
class DoubleDataset(Dataset):
    """Dataset for DeQA with original and processed images."""

    def __init__(
        self,
        data_path: str | list,
        processor: transformers.ProcessorMixin,
        data_args: DeQADataArguments,
        model_id,
        padding=True,
    ):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        level_probs = sources.get("level_probs", None)
        ...
        if "image" in sources:
            ...
            ori_image_folder = self.data_args.ori_image_folder
            res_image_folder = self.data_args.res_image_folder

            assert len(image_files) == 2

            images = [
                get_image_info(
                    os.path.join(ori_image_folder, image_files[0]),
                ...
                get_image_info(
                    os.path.join(res_image_folder, image_files[1]),
                    self.image_min_pixel,
                    self.image_max_pixel,
                    self.image_resized_w,
                    self.image_resized_h,
...
class DataCollatorForDoubleDataset(object):
...
def make_double_data_module(model_id, processor, data_args):
    """Make dataset and collator for supervised fine-tuning."""
    double_dataset = DoubleDataset(
        data_path=data_args.data_path,
        processor=processor,
        data_args=data_args,
        model_id=model_id,
    )
    data_collator = DataCollatorForDoubleDataset(
        pad_token_id=processor.tokenizer.pad_token_id
    )

    return dict(
        train_dataset=double_dataset, eval_dataset=None, data_collator=data_collator
    )
